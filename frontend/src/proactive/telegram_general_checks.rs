...
#[function_component(TelegramGeneralChecks)]
pub fn telegram_general_checks(props: &Props) -> Html {
    let default_checks = "
        ...
        use_effect_with_deps(move |_| {
            spawn_local(async move {
                if let Some(token) = window()
                    ...
                    .flatten()
                {
                    let response = Request::get(&format!("{}/api/profile/telegram-general-checks", config::get_backend_url()))
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                    ...
                    .flatten()
                {
                    ...
                    let response = Request::post(&format!("{}/api/profile/telegram-general-checks", config::get_backend_url()))
                        .header("Authorization", &format!("Bearer {}", token))
                        .json(&request_body)
...
html! {
        <div class="telegram-checks-container">
                ...
                if *is_editing {
                    <div class="edit-section">
                        <textarea
                            ...
                            value={(*checks).clone()}
                            onchange={on_change}
                            placeholder="Enter your custom Telegram message analysis steps..."
                        />
            ...
            <style>
                ...
                .telegram-checks-header {
                    margin-bottom: 2rem;
                    text-align: left;
                }

                .telegram-checks-header h3 {
                    color: #7EB2FF;
                    font-size: 1.5rem;
