                    ...
                    if entry.entry_type == "message" || entry.entry_type == "screened" {
                        if let Some(ref u) = entry.urgency {
                            <span style={match u.as_str() {
                                // "now" tier (and legacy critical/high) = alerting red
                                "now" | "critical" | "high" => "color: #ff6b6b; font-size: 0.65rem; margin-left: 0.4rem; padding: 1px 5px; border-radius: 3px; background: rgba(255,107,107,0.15); font-weight: 600;",
                                // "later" tier (and legacy medium/low) = quiet grey
