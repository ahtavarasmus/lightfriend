...
pub async fn check_user_credits(
    ...
    let mut required_credits_sub: f32 = 0.0; // for voice sub
    if event_type == "message" {
        ...
        required_credits = std::env::var("MESSAGE_COST")
            ...
            .parse::<f32>()
            .unwrap_or(0.15);
        required_credits_sub = std::env::var("MESSAGE_COST_SUB")
            .expect("MESSAGE_COST_SUB not set")
            .parse::<f32>()
            .unwrap_or(0.10);
    }


    // First check if user has sub credits left
    if user.credits_left > required_credits_sub {
        // User has sub credits, they can continue
        return Ok(());
    }
...
pub fn deduct_user_credits(
    ...
    let message_cost_sub = std::env::var("MESSAGE_COST_SUB")
        .expect("MESSAGE_COST_SUB not set")
        .parse::<f32>()
        .unwrap_or(0.10);

    let mut credits_cost = message_cost;
    if user.discount && user.sub_tier == Some("tier 1".to_string()) { // use the sub price always for discounted users
        credits_cost = message_cost_sub; 
    }
    let mut credits_cost_sub: f32 = message_cost_sub;

    if event_type == "voice" {
        let mut voice_second_cost = std::env::var("VOICE_SECOND_COST")
            .expect("VOICE_SECOND_COST not set")
            .parse::<f32>()
            .unwrap_or(0.0033);
        let voice_second_cost_sub = std::env::var("VOICE_SECOND_COST_SUB")
            .expect("VOICE_SECOND_COST_SUB not set")
            .parse::<f32>()
            .unwrap_or(0.0025);
        if user.discount && user.sub_tier == Some("tier 1".to_string()) { // use the sub price always for discounted users
            voice_second_cost = voice_second_cost_sub;
        }
        credits_cost = voice_seconds.unwrap_or(0) as f32 * voice_second_cost;
        credits_cost_sub = voice_seconds.unwrap_or(0) as f32 * voice_second_cost_sub;
    }
    ...
    if user.credits_left > 0.00 {
        let new_credits_left = user.credits_left - credits_cost_sub;
