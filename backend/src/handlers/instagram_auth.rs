...
async fn connect_instagram_with_retry(
    ...
    for retry_count in 0..MAX_RETRIES {
        tracing::debug!(
            "Attempt {}/{} to connect Instagram for user {}",
            retry_count + 1,
            MAX_RETRIES,
            user_id
        );
        match connect_instagram(client, bridge_bot, curl_paste).await {
            Ok(result) => {
                tracing::info!(
                    "Successfully connected Instagram on attempt {}/{} for user {}",
                    retry_count + 1,
                    MAX_RETRIES,
                    user_id
                );
            ...
            Err(e) => {
                tracing::error!(
                    "Error during Instagram connection attempt {}/{} for user {}: {}",
                    retry_count + 1,
                    MAX_RETRIES,
                    user_id,
                    e
...
async fn connect_instagram(
    ...
    // Send the cURL paste
    tracing::info!(
        "📤 Sending cURL paste for authentication (length: {})",
        curl_paste.len()
    );
...
pub async fn start_instagram_connection(
    ...
    tracing::info!(
        "Received POST request to start Instagram connection for user {}",
        auth_user.user_id
    );
    tracing::debug!("Request body: curl_paste length = {}", req.curl_paste.len());
    tracing::debug!(
        "🚀 Starting Instagram connection process for user {}",
        auth_user.user_id
    );
    ...
    let bridge_bot = std::env::var("INSTAGRAM_BRIDGE_BOT").map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    tokio::spawn(async move {
        tracing::debug!(
            "Spawning monitor task for Instagram connection for user {}",
            auth_user.user_id
        );
        ...
        .await
        {
            Ok(_) => {
                tracing::info!(
                    "Instagram connection monitoring completed successfully for user {}",
                    auth_user.user_id
                );
            }
            Err(e) => {
                tracing::error!(
                    "Instagram connection monitoring failed for user {}: {}",
                    auth_user.user_id,
                    e
                );
            }
    ...
    tracing::info!(
        "Instagram connection process initiated successfully for user {}",
        auth_user.user_id
    );
    Ok(AxumJson(InstagramConnectionResponse {
...
pub async fn get_instagram_status(
    ...
    tracing::debug!(
        "📊 Checking Instagram status for user {}",
        auth_user.user_id
    );
    let bridge = state
        .user_repository
...
async fn monitor_instagram_connection(
    ...
    tracing::debug!(
        "👀 Starting optimized Instagram connection monitoring for user {} in room {}",
        user_id,
        room_id
    );
    let bot_user_id = OwnedUserId::try_from(bridge_bot)?;
    ...
    for attempt in 1..60 {
        ...
        match client.sync_once(sync_settings.clone()).await {
            Ok(_) => tracing::debug!(
                "Sync successful on attempt {} for user {}",
                attempt,
                user_id
            ),
            Err(e) => tracing::warn!(
                "Sync failed on attempt {} for user {}: {}",
                attempt,
                user_id,
                e
            ),
                    ...
                    if event.sender() == bot_user_id {
                        ...
                        ) = event
                        {
                            ...
                            if content.to_lowercase().contains("successful login") {
                                tracing::info!(
                                    "🎉 Instagram successfully connected for user {}",
                                    user_id
                                );
                                ...
                                tracing::debug!(
                                    "Creating new connected bridge record for Instagram"
                                );
                            ...
                            {
                                tracing::error!(
                                    "❌ Instagram connection failed for user {}: {}",
                                    user_id,
                                    content
                                );
                                state.user_repository.delete_bridge(user_id, "instagram")?;
    ...
    tracing::error!(
        "Instagram connection timed out for user {} after 3 minutes",
        user_id
    );
    state.user_repository.delete_bridge(user_id, "instagram")?;
...
pub async fn resync_instagram(
    ...
    tracing::info!(
        "🔄 Starting Instagram resync process for user {}",
        auth_user.user_id
    );
    ...
    if client.get_room(&room_id).is_some() {
        ...
        tracing::debug!(
            "📱 No specific resync commands for Instagram, as portals are created dynamically"
        );

        tracing::debug!(
            "✅ Instagram resync process completed for user {}",
            auth_user.user_id
        );
        Ok(AxumJson(json!({
        ...
        tracing::error!(
            "Instagram bridge room not found for user {}",
            auth_user.user_id
        );
        Err((
...
pub async fn disconnect_instagram(
    ...
    tracing::debug!(
        "🔌 Starting Instagram disconnection process for user {}",
        auth_user.user_id
    );
    // Get the bridge information first
    let bridge = state
    ...
    if let Some(room) = client.get_room(&room_id) {
        ...
        tracing::warn!(
            "Instagram bridge room not found during disconnection for user {}",
            auth_user.user_id
        );
    }
    // Delete the bridge record
    tracing::debug!(
        "Deleting Instagram bridge record for user {}",
        auth_user.user_id
    );
    state
        .user_repository
        .delete_bridge(auth_user.user_id, "instagram")
        .map_err(|e| {
    ...
    tracing::debug!(
        "✅ Instagram disconnection completed for user {}",
        auth_user.user_id
    );
    Ok(AxumJson(json!({
