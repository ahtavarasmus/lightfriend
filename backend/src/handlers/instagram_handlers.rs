...
pub async fn test_fetch_instagram_messages(
    ...
    match crate::utils::bridge::fetch_bridge_messages("instagram", &state, start_time, false).await {
        Ok(messages) => {
        ...
        Err(e) => {
            ...
            match fetch_bridge_messages("instagram", &state, start_time, false).await {
...
pub async fn search_instagram_rooms_handler_query(
    State(state): State<std::sync::Arc<AppState>>,
    auth_user: AuthUser,
    Query(params): Query<SearchQuery>,
) -> Result<Json<Vec<BridgeRoom>>, StatusCode> {
    match crate::utils::bridge::search_bridge_rooms("instagram", &state, auth_user.user_id, &params.search).await {
        Ok(rooms) => Ok(Json(rooms)),
        Err(e) => {
            tracing::error!("Failed to search Instagram rooms for user {}: {}", auth_user.user_id, e);
