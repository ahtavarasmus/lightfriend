    ...
    tokio::spawn(async move {
        ...
        if let Ok(room_id) = OwnedRoomId::try_from(room_id_str.as_str()) {
            if let Some(room) = client.get_room(&room_id) {
                ...
                // "Logged out" reply. Bare `!signal logout` is a silent
                // no-op (prints usage message).
                let bridge_bot_env =
                    std::env::var("SIGNAL_BRIDGE_BOT").expect("SIGNAL_BRIDGE_BOT not set");
                    ...
                    matrix_sdk::ruma::OwnedUserId::try_from(bridge_bot_env.as_str())
                {
                    ...
                    {
                        Ok(n) => tracing::info!(
                            "Signal background cleanup: logged out {} login(s) for user {}",
                            n,
                            user_id
                        ),
                        Err(e) => {
...
pub async fn check_signal_health(
    ...
    // Send `!signal list-logins` (read-only). The bridge bot responds with one
    // line per login: "* `<id>` (<identifier>) - `CONNECTED`" for healthy
    // logins. Other statuses indicate unhealthy state.
    tracing::info!(
        "📤 Sending !signal list-logins for health check for user {}",
        auth_user.user_id
    );
    let responses = match crate::utils::bridge::probe_bridge_room(
        &client,
    ...
    if responses.is_empty() {
        tracing::warn!(
            "⚠️ Signal health check: no response from bridge bot for user {}",
            auth_user.user_id
        );
        return Ok(AxumJson(json!({
            "healthy": false,
    ...
    if crate::utils::bridge_responses::any_connected(&combined) {
        ...
        if let Some(ident) = crate::utils::bridge_responses::first_connected_identifier(&combined) {
            if let Err(e) =
                state
                    .user_repository
                    .update_bridge_data(auth_user.user_id, "signal", &ident)
            {
        ...
        // No CONNECTED login. Report unhealthy but DO NOT delete the bridge -
        // that's the user's call. Frontend can prompt them to reconnect.
        tracing::warn!(
            "❌ Signal health check: no CONNECTED login for user {}: {:?}",
            auth_user.user_id,
            combined
        );
        Ok(AxumJson(json!({
