                    ...
                    Ok(None) if event.type_ == stripe::EventType::CustomerSubscriptionCreated => {
                        ...
                        match state.user_core.find_by_email(&email) {
                            Ok(Some(existing_user)) => {
                                ...
                                let _ = state
                                    .user_repository
                                    .set_stripe_customer_id(existing_user.id, customer_id.as_ref());

                                // Update phone if they provided one and don't have one set
                                if !phone.is_empty() && existing_user.phone_number.is_empty() {
                                    let _ = state
                                        .user_core
                                        .update_phone_number(existing_user.id, &phone);
                                    // Also set phone country
                                    if let Ok(Some(country)) =
                                        crate::handlers::profile_handlers::set_user_phone_country(
                                            &state,
                                            existing_user.id,
                                            &phone,
                                        )
                                        .await
                                    {
                                        let _ = state.user_core.set_preferred_number_for_country(
                                            existing_user.id,
                                            &country,
                                        );
                                    }
                            ...
                            Ok(None) => {
                                ...
                                tracing::info!(
                                    "Created new user {} from guest checkout",
                                    created_user.id
                                );
                                ...
                                let _ = state
                                    .user_repository
                                    .set_stripe_customer_id(created_user.id, customer_id.as_ref());
                                ...
                                tokio::spawn(async move {
                                    if let Err(e) = crate::utils::email::send_magic_link_email(
                                        &email_clone,
                                        &magic_link,
                                    )
                                    .await
