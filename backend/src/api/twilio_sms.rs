...
fn resolve_sms_user(state: &Arc<AppState>, from: &str) -> Result<User, SmsResult> {
...
async fn check_sms_access(state: &Arc<AppState>, user: &User) -> Result<(), SmsResult> {
...
async fn handle_sms_early_response(
    state: &Arc<AppState>,
    user: &User,
    body: &str,
    skip_twilio_send: bool,
    ...
    // Important-alert feedback path: after a recent urgent alert, a bare
    // "1"/"2" records whether the interruption was useful. Commitment replies
    // get first priority above because they have their own pending prompt.
...
pub async fn process_sms(
    ...
    let user = match resolve_sms_user(state, &payload.from) {
        Ok(user) => user,
        Err(result) => return result.into_response(),
    };

    if let Err(result) = check_sms_access(state, &user).await {
    ...
    if let Some(response) = handle_sms_early_response(
        state,
        &user,
        &payload.body,
